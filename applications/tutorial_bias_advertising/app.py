...
# Define request model using Pydantic for data validation
class BiasRequestModel(BaseModel):
    target_label_name: str
    scores_name: str
    random_seed: Optional[int] = 1001

    # Ensure label and scores names are not empty
    @validator('target_label_name', 'scores_name', pre=True, always=True)
...
ad_conversion_dataset = load_data('bias-in-advertising/ad_campaign_data.csv')
...
def convert_to_standard_dataset(df, target_label_name, scores_name=""):
    ...
    standard_dataset = StandardDataset(
        df=df, label_name=target_label_name, favorable_classes=favorable_target_label,
        scores_name=scores_name, protected_attribute_names=protected_attributes,
        privileged_classes=privileged_classes, categorical_features=categorical_features,
...
# Streaming generator function for bias measurement and mitigation
def generate_response(data):
    target_label_name = data.target_label_name
    scores_name = data.scores_name
    random_seed = data.random_seed

    logger.info(f"Received request with target_label_name: {target_label_name}, scores_name: {scores_name}, random_seed: {random_seed}")
    ...
    metric_orig = BinaryLabelDatasetMetric(ad_standard_dataset_orig, unprivileged_groups=unprivileged_groups, privileged_groups=privileged_groups)
    disparate_impact_orig = metric_orig.disparate_impact()
    yield f"Disparate impact (original): {disparate_impact_orig}\n"

    metric_pred = BinaryLabelDatasetMetric(ad_standard_dataset_pred, unprivileged_groups=unprivileged_groups, privileged_groups=privileged_groups)
    ...
    metric_pred_valid_transf = BinaryLabelDatasetMetric(dataset_transf_pred_valid, unprivileged_groups=unprivileged_groups, privileged_groups=privileged_groups)
